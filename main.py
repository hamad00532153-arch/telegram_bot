...
# ============================================================
# IMPORTANT:
# Token ـی نوێی Telegram لێرە دابنێ
...
# فایل بۆ ئەوەی هەواڵە دووبارەکان پۆست نەکرێن
SEEN_FILE = "seen_news.json"
# هەر 5 خولەک جارێک پشکنین
CHECK_INTERVAL = 300
# Gemini model
...
if (
    not TELEGRAM_TOKEN
    or TELEGRAM_TOKEN == "PUT_YOUR_NEW_TELEGRAM_BOT_TOKEN_HERE"
):
    raise ValueError(
        "❌ تکایە Telegram Bot Token ـەکەت دابنێ."
    )
if (
    not GEMINI_API_KEY
    or GEMINI_API_KEY == "PUT_YOUR_NEW_GEMINI_API_KEY_HERE"
):
    raise ValueError(
        "❌ تکایە Gemini API Key ـەکەت دابنێ."
    )
# ============================================================
# CLIENTS
# ============================================================
...
def save_seen_news(seen):
    try:
        # تەنها 500 هەواڵی دواوە هەڵدەگرێت
        data = list(seen)[-500:]
        with open(
            SEEN_FILE,
            "w",
...
def translate_and_format(
    ...
    prompt = f"""
تۆ نووسەرێکی پڕۆفیشناڵی هەواڵی وەرزشییت.
ئەم هەواڵەی خوارەوە وەرگێڕە بۆ زمانی کوردیی سۆرانی
و بۆ پۆستی Telegram بە شێوەیەکی زۆر پڕۆفیشناڵ ئامادەی بکە.
مەرجەکان:
...
async def send_news(news):
    ...
    if news["image"]:
        try:
            # Caption limit لە Telegram کەمترە لە message
            caption = message
            if len(caption) > 1024:
                caption = caption[:900] + (
                    f"\n\n🔗 سەرچاوە:\n{source_link}"
